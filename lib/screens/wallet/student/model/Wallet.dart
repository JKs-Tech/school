...
class Wallet {
  ...
  factory Wallet.fromJson(Map<String, dynamic> json) => Wallet(
    ...
    myBalance: json["myBalance"]?.toString() ?? '0',
    paymentMethods:
    ...
    bankAccounts:
        json["bankAccounts"] == null
            ? []
            : List<BankAccount>.from(
              json["bankAccounts"].map((x) => BankAccount.fromJson(x)),
            ),
    walletTransactions:
        json["walletTransactions"] == null
            ? []
            : List<WalletTransaction>.from(
              json["walletTransactions"].map(
                (x) => WalletTransaction.fromJson(x),
              ),
            ),
    stripeInfo:
        json["stripe_info"] == null
            ? null
            : StripeInfo.fromJson(json["stripe_info"]),
