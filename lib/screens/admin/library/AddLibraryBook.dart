...
class _AddAdminBookState extends State<AddAdminBook> {
    ...
    Utils.getStringValue('id').then((value) {
      ...
      Utils.getStringValue('token').then((value) {
        setState(() {
          ...
          //initial date
          initDateTime =
              '${date?.year}-${getAbsoluteDate(date?.month ?? 1)}-${getAbsoluteDate(date?.day ?? 1)}';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: 'Add Book'),
          ...
          child: Padding(
            ...
            child: Column(
              children: <Widget>[
                              ...
                              setState(() {
                                setState(() {
                                  ...
                                  _selectedaAssignDate =
                                      '${_dateTime?.year}-${getAbsoluteDate(_dateTime?.month ?? 1)}-${getAbsoluteDate(_dateTime?.day ?? 1)}';
                ...
                BottomLine(),
                Container(
                  padding: EdgeInsets.only(bottom: 16.0, top: 100.0),
  ...
  Widget getSubjectsDropdown(List<AdminSubject> subjects) {
    return SizedBox(
      ...
      child: DropdownButton(
        ...
        style: Theme.of(
          context,
        ).textTheme.headlineMedium?.copyWith(fontSize: 13.0),
        onChanged: (value) {
          setState(() {
            //_selectedClass = value;
            selectedSubject = value;
            selectedSubjectId = getCode(subjects, value as String);
          });
        },
      ...
      child: DropdownButton(
        ...
        onChanged: (value) {
          setState(() {
            ...
            selectedCategoryId = getCode(categories, value as String);
          });
        },
  ...
  Future<AdminSubjectList> getAllSubject() async {
    final response = await http.get(
      Uri.parse(InfixApi.subjectList),
  ...
  Future<AdminCategoryList> getAllCategory() async {
    final response = await http.get(
      Uri.parse(InfixApi.bookCategory),
      headers: Utils.setHeader(_token.toString()),
    );
