...
class ChatGroupOpenController extends GetxController {
  ...
  Rx<GroupRole> selectedGroupRole = GroupRole(name: "User", role: 0).obs;
  ...
  Future<ChatGroupOpenModel> getAll() async {
    // Logger.warn(groupId);
    ChatGroupOpenModel sourceData = ChatGroupOpenModel();
    try {
  ...
  Future chatGroupSetRead(type) async {
    ...
    try {
      Map jsonData = {"type": type, "group_id": groupId};
  ...
  Future deleteChatGroup(groupId) async {
    try {
      await getIdToken().then((value) async {
        final response = await http.post(
          Uri.parse(InfixApi.chatGroupDelete),
          headers: Utils.setHeader(_token.toString()),
  ...
  Future removePeople(userId) async {
    ...
    try {
      Map jsonData = {"user_id": userId, "group_id": groupId};
  ...
  Future addPeople() async {
    EasyLoading.show(status: 'Updating...');
    try {
      Map jsonData = {"group_id": groupId, "user_id": selectedAddUser.value.id};
  ...
  Future leaveGroup() async {
    ...
    try {
      Map jsonData = {"group_id": groupId, "user_id": id.value};
...
class GroupRole {
  ...
  GroupRole({required this.role, required this.name});
}
