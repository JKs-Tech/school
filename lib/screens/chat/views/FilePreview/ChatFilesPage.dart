...
class ChatFilesPage extends StatefulWidget {
  final String chatId;
  final bool isGroup;
  const ChatFilesPage({super.key, required this.chatId, required this.isGroup});
  @override
...
class _ChatFilesPageState extends State<ChatFilesPage> {
  ...
  void initState() {
    if (widget.isGroup) {
      _chatFilesController = Get.put(
        ChatFilesController(widget.chatId, 'group'),
      );
    } else {
      _chatFilesController = Get.put(
        ChatFilesController(widget.chatId, 'single'),
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: "Files"),
      body: Obx(() {
        if (_chatFilesController?.isLoading.value ?? false) {
