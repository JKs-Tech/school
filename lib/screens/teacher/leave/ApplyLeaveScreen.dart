...
class _ApplyLeaveScreenState extends State<ApplyLeaveScreen> {
          ...
          leaveAvailable
              ? GestureDetector(
                ...
                onTap: () {
                  ...
                  if (reason.isNotEmpty && (_file?.path.isNotEmpty ?? false)) {
                    setState(() {
                      isResponse = true;
                    });
                    uploadLeave();
  ...
  Future<LeaveList> getAllLeaveType(id) async {
    final response = await http.get(
      Uri.parse(InfixApi.userLeaveType(_id)),
  ...
  void sentNotificationTo() async {
    final response = await http.get(
      Uri.parse(
        InfixApi.sentNotificationForAll(
          1,
          'Leave',
          'New leave request has come',
        ),
      ),
    );
    if (response.statusCode == 200) {}
