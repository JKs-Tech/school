...
class _AddHomeworkScrrenState extends State<AddHomeworkScrren> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: 'Add Homework'),
  ...
  Widget getClassDropdown(List<Classes> classes) {
    return Container(
      ...
      child: DropdownButton(
        ...
        style: Theme.of(
          context,
        ).textTheme.headlineMedium?.copyWith(fontSize: ScreenUtil().setSp(14)),
        onChanged: (value) {
          setState(() {
  ...
  Widget getSubjectDropdown(List<TeacherSubject> subjectList) {
    return Container(
      ...
      child: DropdownButton(
        ...
        style: Theme.of(
          context,
        ).textTheme.headlineMedium?.copyWith(fontSize: 15.0),
        onChanged: (value) {
          setState(() {
            _selectedSubject = value;
            subjectId = getSubjectId(subjectList, value.toString());
          });
        },
  ...
  Future<TeacherSubjectList> getAllSubject(int id) async {
    final response = await http.get(
      Uri.parse(InfixApi.getTeacherSubject(id)),
      headers: Utils.setHeader(_token.toString()),
    );
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
