...
class _FeesInvoiceScreenState extends State<FeesInvoiceScreen> {
  ...
  Future<FeesRecordList> getFeesInvoice() async {
    final response = await http.get(
      Uri.parse(InfixApi.adminFeesInvoiceList),
                      ...
                      () => showSearch(
                        ...
                        delegate: SearchPage<FeesRecord>(
                          ...
                          suggestion: const Center(
                            child: Text(
                              'Filter people by name, class or section',
                            ),
                          ),
                          searchStyle:
                          ...
                          failure: Center(child: Utils.noDataWidget()),
                          filter:
                              (record) => [
                                record.student ?? '',
                                record.recordClass ?? '',
                          ...
                          builder:
                              (record) => ListTile(
                                onTap: () {
                                  Get.to(
                                    () => FeeInvoiceDetailsView(
                                      invoiceId: record.id ?? 0,
                                    ),
                                  );
                                },
                                title: Text(
                                  "${record.student} (${record.recordClass} - ${record.section})" ??
                                      'NA',
              ...
              if (snapshot.hasData) {
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      title: Text(
                        "${feeRecord.student} (${feeRecord.recordClass} - ${feeRecord.section})" ??
                            'NA',
                        style: Theme.of(
                          context,
                                ...
                                onSelected: (String value) async {
                                  if (value == 'view') {
                                    ...
                                    final response = await http.post(
                                      ...
                                      body: jsonEncode({'id': feeRecord.id}),
                                    );
                                    if (response.statusCode == 200) {
      ...
      return Container(
        ...
        child: Padding(
          ...
          child: Text(
            ...
            : double.parse(feesRecord.paidAmount?.toString() ?? '0'))! >
        0.0) {
      return Container(
        width: MediaQuery.of(context).size.width,
