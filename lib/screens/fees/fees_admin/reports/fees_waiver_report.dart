...
class _AdminFeesWaiverReportState extends State<AdminFeesWaiverReport> {
  ...
  Future<FeeWaiverReportModel> getSearchData(Map data) async {
    final response = await http.post(
      Uri.parse(InfixApi.adminFeesWaiverSearch),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: "Waiver Report"),
      body: ListView(
          ...
          FutureBuilder<FeeWaiverReportModel>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (snapshot.hasData) {
                  if (snapshot.data?.waiverReport?.isEmpty ?? true) {
                    return Utils.noDataWidget();
                  } else {
                    var total = 0.0;
                    for (var element
                        in snapshot.data?.waiverReport?.values ??
                            <WaiverReport>[]) {
                      total += element.waiver ?? 0.0;
                    }
                      ...
                      children: [
                        ...
                        ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            WaiverReport report =
                                snapshot.data?.waiverReport?.values.elementAt(
                                  index,
                                ) ??
                                WaiverReport();
