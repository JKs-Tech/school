...
class _AdminFeesBalanceReportState extends State<AdminFeesBalanceReport> {
  ...
  Future<FeesBalanceReportModel> getSearchData(Map data) async {
    final response = await http.post(
      Uri.parse(InfixApi.adminFeesBalanceSearch),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: "Balance Report"),
      body: ListView(
          ...
          FutureBuilder<FeesBalanceReportModel>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (snapshot.hasData) {
                  if (snapshot.data?.balanceReport?.isEmpty ?? true) {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        double.parse(
                          paymentReport.balance.toString(),
                        ).toStringAsFixed(2),
                        style: Theme.of(
                          context,
                        ).textTheme.headlineMedium?.copyWith(fontSize: 12),
