...
class _AdminFeesFineReportState extends State<AdminFeesFineReport> {
  ...
  Future<FeeFineReportModel> getSearchData(Map data) async {
    final response = await http.post(
      Uri.parse(InfixApi.adminFeesFineSearch),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBarWidget(title: "Fine Report"),
      body: ListView(
          ...
          FutureBuilder<FeeFineReportModel>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (snapshot.hasData) {
                  if (snapshot.data?.fineReport?.isEmpty ?? true) {
                    return Utils.noDataWidget();
                  } else {
                    var total = 0.0;
                    for (var element
                        in snapshot.data?.fineReport?.values ??
                            <FineReport>[]) {
                      total += (element.fine ?? 0.0);
                    }
                    return Column(
                      children: [
                        Container(
                        ...
                        ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            FineReport report =
                                snapshot.data?.fineReport?.values.elementAt(
                                  index,
                                ) ??
                                FineReport();
