...
class AdminFeesController extends GetxController {
    ...
    await getIdToken().then((value) async {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          feesAdminAddPaymentModel.value = FeesAdminAddPaymentModel.fromJson(
            jsonData,
          );
          ...
          feesAdminAddPaymentModel.value.paymentMethods?.insert(
            0,
          ...
          for (var element
              in feesAdminAddPaymentModel.value.invoiceDetails ?? []) {
  ...
  Future processPayment(
    ...
    try {
      ...
      await _dio
          .post(
            InfixApi.adminFeesAddPaymentStore,
            data: formData,
            options: dio.Options(
              headers: Utils.setHeader(_token.value.toString()),
            ),
