                        ...
                        : ElevatedButton(
                          ...
                          onPressed: () async {
                            if (formKey.currentState?.validate() ?? false) {
                              ...
                              await paymentDataSave("Xendit").then((
                                value,
                              ) async {
                                setState(() {
      ...
      if (response.statusCode == 200) {
        ...
        if (body['status'] == "IN_REVIEW") {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder:
                  (context) => XenditPaymentScreen(
                    payment: PaymentMethod(method: widget.method),
                    fee: widget.fee,
                    amount: widget.amount,
        ...
        if (body['error_code'] == "AMOUNT_BELOW_MINIMUM_LIMIT") {
          ...
          final xenditErrorResponse = xenditErrorResponseFromJson(
            response.body,
          );
