...
class FeesDueReportModel {
  FeesDueReportModel({this.feesDues});

  List<FeesDue>? feesDues;

  factory FeesDueReportModel.fromJson(Map<String, dynamic> json) =>
      FeesDueReportModel(
        feesDues: List<FeesDue>.from(
          json["feesDues"].map((x) => x == null ? null : FeesDue.fromJson(x)),
        ),
      );

  Map<String, dynamic> toJson() => {
    "feesDues": List<dynamic>.from((feesDues ?? []).map((x) => x.toJson())),
  };
}
