...
class _SubjectStudentAttendanceScreenState
  ...
  String schoolId;
  String subjectName;
  late Future<StudentAttendanceList> attendances;
  ...
  void didChangeDependencies() async {
    ...
    if (id != null) {
      await Utils.getStringValue('schoolId')
          .then((value) {
            setState(() {
              schoolId = value;
            });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomScreenAppBarWidget(title: '$subjectName Attendance'),
        ...
        if (snapshot.hasData) {
          return Row(
            children: <Widget>[
              ...
              Text(
                getStatusCount(titleVal, snapshot.data?.attendances ?? []),
  ...
  String getStatus(
    ...
    String status = '';
    for (int a = i; a <= j; a++) {
      if (int.parse(AppFunction.getDay(attendances[a].date ?? "") ?? '0') ==
          date) {
