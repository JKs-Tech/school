    ...
    Utils.getStringValue('id').then((value) {
      setState(() {
        ...
        examSchedule.then((val) {
          setState(() {
            _selected =
                val.examTypes?.isNotEmpty == true
                    ? val.examTypes![0].title
                    : '';
            examTypeId =
                val.examTypes?.isNotEmpty == true ? val.examTypes![0].id! : 0;
    ...
    return Scaffold(
      ...
      body: FutureBuilder<ExamSchedule>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data?.examTypes?.isNotEmpty == true) {
              return Column(
                children: <Widget>[
        ...
        children: [
          StudentRecordWidget(
            onTap: (Record record) {
              ...
              setState(() {
                results = getAllClassExamResult(id, examId, record.id!);
              });
            },
          ),
          Expanded(
