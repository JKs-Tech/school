...
class TakeExamScreen extends StatefulWidget {
  ...
  const TakeExamScreen({super.key, required this.takeExamModel});
...
class _TakeExamScreenState extends State<TakeExamScreen> {
  final OnlineExamController _questionController = Get.put(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              return GestureDetector(
                ...
                child:
                    qnController.answered[index] == true
                        ? Container(
                          width: 40,
                          height: 40,
                        ...
                        : Container(
                          ...
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                            gradient:
                                qnController.checkSelected(index)
                                    ? LinearGradient(
                                      colors: [
                                        Color(0xff7C32FF),
                                        Color(0xffC738D8),
...
class QuestionCard extends StatefulWidget {
  const QuestionCard({super.key, required this.assign, required this.index});
...
class _QuestionCardState extends State<QuestionCard>
  ...
  final List<String> _choicesList = ['True', 'False'];
  ...
  Future onItemClicked(CheckboxModal ckbItem) async {
    setState(() {
      ckbItem.value = !(ckbItem.value ?? false);
    });
    if (ckbItem.value ?? false) {
      if (ckbItem.id != null) {
  ...
  void initState() {
    if (widget.assign.questionType == "M") {
      ...
      for (var element in widget.assign.miQuestions ?? []) {
        multipleImageList.add(
          CheckboxModal(image: element.miTitle, id: element.miId, value: false),
        );
      }
    } else if (widget.assign.questionType == "IMQ") {
      for (var element in widget.assign.imqQuestions ?? []) {
        imageQuestionList.add(
          CheckboxModal(
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          TextFormField(
                            ...
                            enabled:
                                !qnController.answered[widget.index]
                                    ? true
                                    : false,
                            validator: (value) {
                                  ...
                                  assignIds.contains(
                                      ...
                                      ? Container(
                                        ...
                                        decoration: BoxDecoration(
                                          ...
                                          color: Color.fromARGB(99, 35, 31, 46),
                                        ),
                                        child: Icon(
                                          Icons.check,
                                          color: Colors.white,
...
class ContinueSkipSubmitBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      child:
          qnController.submitSingleAnswer.value
              ? Align(
              ...
              : Align(
                alignment: Alignment.bottomCenter,
                child:
                    qnController.lastQuestion.value
                        ? Container(
                          height: 40,
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(5.0),
...
class PhotoViewerWidget extends StatelessWidget {
  ...
  const PhotoViewerWidget({super.key, required this.image});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: PhotoView(
        imageProvider: NetworkImage('${AppConfig.domainName}/$image'),
        loadingBuilder:
...
class QuestionTypeWidget extends StatelessWidget {
  final String title;
  const QuestionTypeWidget({super.key, required this.title});
