...
class OnlineExamController extends GetxController
  ...
  Animation get animation => _animation ?? AnimationController(vsync: this);
  ...
  void startController(TakeExamModel quizParam) {
    ...
    if (quiz.value.onlineExam?.durationType == "question") {
      questionTime.value = quiz.value.onlineExam?.defaultQuestionTime ?? 1;
    } else {
      questionTime.value = quiz.value.onlineExam?.duration ?? 1;
    }
    ...
    _animationController = AnimationController(
      duration: Duration(seconds: questionTime.value * 60),
      vsync: this,
    );
    _animation = StepTween(
      begin: questionTime.value * 60,
      end: 0,
      ...
      if (response.statusCode == 200) {
        ...
        Utils.getStringValue('id').then((value) async {
          ...
          await _examController.getAllActiveExam(
            id,
            userController.selectedRecord.value.id ?? 0,
          );
        });
  ...
  Future skipPress(index) async {
    ...
    if (_questionNumber.value != quiz.value.examQuestions?.length) {
      currentQuestion.value =
          quiz.value.examQuestions?[index + 1 ?? 0] ?? ExamQuestion();
  ...
  Future startQuiz(examId, recordId, String schoolId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          InfixApi.takeOnlineExamModule(examId, recordId, int.parse(schoolId)),
        ),
