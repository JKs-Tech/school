...
class LeaveListStudent extends StatefulWidget {
  ...
  LeaveListStudent({super.key, required this.id});
...
class _LeaveListStudentState extends State<LeaveListStudent> {
          ...
          children: [
            ...
            Container(
              child: FutureBuilder<StudentMyLeavesList>(
                future: myLeaves as Future<StudentMyLeavesList>?,
                          ...
                          return Padding(
                            ...
                            child: Row(
                              children: <Widget>[
                                Expanded(
                                  child: Text(
                                    snapshot.data?.studentMyLeave[index].type ??
                                        'N/A',
                                    maxLines: 1,
                                    style:
                                        Theme.of(
                                ...
                                Expanded(
                                  child: Text(
                                    '${snapshot.data?.studentMyLeave[index].days} ${"days".tr}',
                                    maxLines: 1,
                                    style:
                                        Theme.of(
                                          context,
                    ...
                    builder: (context) {
                      ...
                      return Scaffold(
                        ...
                        body: Padding(
                          ...
                          child: TabBarView(
                            children: [
                              getLeaves(pendingLeaves ?? Future.value()),
                              getLeaves(approvedLeaves ?? Future.value()),
                              getLeaves(rejectedLeaves ?? Future.value()),
                            ],
                          ),
                        ),
                      );
  ...
  Widget getLeaves(Future future) {
    return SizedBox(
      ...
      child: FutureBuilder<LeaveAdminList>(
        future: future as Future<LeaveAdminList>,
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data?.leaves.isNotEmpty ?? false) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return InkWell(
                    onTap: () {
                      showAlertDialog(context, data?[index] ?? LeaveAdmin());
                    },
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
  ...
  Future<StudentMyLeavesList> getMyLeaves(var id) async {
    final response = await http.get(
      Uri.parse(InfixApi.studentApplyLeave(id)),
  ...
  Future<LeaveAdminList> getApprovedLeaves(var id) async {
    final response = await http.get(
      Uri.parse(InfixApi.approvedLeaves(id)),
      headers: Utils.setHeader(_token.toString()),
    );
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
  ...
  Future<LeaveAdminList> getRejectedLeaves(var id) async {
    final response = await http.get(
      Uri.parse(InfixApi.rejectedLeaves(id)),
      headers: Utils.setHeader(_token.toString()),
    );
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
