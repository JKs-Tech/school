...
class EditProfile extends StatefulWidget {
  ...
  const EditProfile({super.key, required this.id, required this.updateData});
...
class _EditProfileState extends State<EditProfile> {
  ...
  Future<StudentDetailsModel> getProfile() async {
    ...
    final response = await http.get(
      Uri.parse(InfixApi.getChildren(widget.id)),
  ...
  Future updateData({
    ...
    var response = await _dio.post(
      ...
      options: DIO.Options(headers: Utils.setHeader(_token.toString())),
      data: formData,
    );
    ...
    if (data['data']['flag'] == true) {
      await getProfile().then((value) {
        Utils.saveStringValue(
          "image",
          value.studentData?.user?.studentPhoto ?? '',
        );
      });
      setState(() {});
