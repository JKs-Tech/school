...
class JitsiVirtualClass extends StatefulWidget {
  final String type;
  const JitsiVirtualClass({super.key, required this.type});
  @override
...
class _JitsiVirtualClassState extends State<JitsiVirtualClass> {
      ...
      setState(() {
        ...
        if (_userController.role.value == "2" ||
          ...
          virtualClass = getAllMeeting(
            recordId: _userController.selectedRecord.value.id ?? 0,
          );
        } else {
          virtualClass = getAllMeeting(recordId: 0);
        }
      });
    });
          ...
          Expanded(
            child: FutureBuilder<VirtualClass>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  if (snapshot.data?.data?.meetings?.isEmpty ?? true) {
  ...
  Future<VirtualClass> getAllMeeting({required int recordId}) async {
    final url =
        widget.type == "class"
            ? Uri.parse(InfixApi.getVirtualClass(recordId, 'jitsi'))
