...
class _StudentLessonsViewState extends State<StudentLessonsView> {
  ...
  Future<List<PlanDetails>> getLessonByDay(
    ...
    try {
      final response = await http.get(
        Uri.parse(
          "${InfixApi.studentLessonPlanByDate}/${widget.id}/$recordId/$date/$dayId",
        ),
        headers: Utils.setHeader(_userController.token.value.toString()),
      );
      if (response.statusCode == 200) {
  ...
  Future<LessonPlan> getPreviousWeek(int recordId, studentId, startDate) async {
    try {
      final response = await http.get(
        Uri.parse(
          "${InfixApi.studentLessonPreviousWeek}/${widget.id}/$recordId/$startDate",
        ),
        headers: Utils.setHeader(_userController.token.value.toString()),
      );
      if (response.statusCode == 200) {
  ...
  Future<LessonPlan> getNextWeek(int recordId, studentId, endDate) async {
    try {
      final response = await http.get(
        Uri.parse(
          "${InfixApi.studentLessonNextWeek}/${widget.id}/$recordId/$endDate",
        ),
        headers: Utils.setHeader(_userController.token.value.toString()),
      );
      if (response.statusCode == 200) {
              ...
              FutureBuilder<LessonPlan>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    if (snapshot.hasData) {
                      if (snapshot.data?.weeks?.isNotEmpty ?? false) {
                        return Builder(
                          builder: (context) {
                            return Column(
                              children: [
                            ...
                            child: SizedBox(
                              ...
                              child: Text(
                                DateFormat('EEE, MMM dd').format(
                                  plan.plan?.lessonDate ?? DateTime.now(),
                                ),
                                style:
                                    Theme.of(context).textTheme.headlineMedium,
                              ),
